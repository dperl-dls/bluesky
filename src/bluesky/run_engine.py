...
class LoggingPropertyMachine(PropertyMachine):
    ...
    def __set__(self, obj, value):
        ...
        state_logger.info(
            "Change state on %r from %r -> %r", obj, old_value, value, extra=tags
        )
...
class RunEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self._call_returns_result = (
            call_returns_result  # should __call__ return UIDs or plan value
        )
        self._run_bundlers: typing.Dict[typing.Any, RunBundler] = (
            {}
        )  # a mapping of open run -> bundlers
        ...
        self._run_start_uids = (
            list()
        )  # run start uids generated by __call__  # noqa: C408
        ...
        self._seen_wait_and_move_on_keys = (
            set()
    ...
    def request_pause(self, defer=False):
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._request_pause_coro(defer), loop=self.loop
        )
        ...
        for obj in self._objs_seen:
            if isinstance(obj, Pausable):
                fut = asyncio.run_coroutine_threadsafe(
                    maybe_await(obj.resume()), self._loop
                )
                fut.result()
    ...
    def request_suspend(
        self, fut, *, pre_plan=None, post_plan=None, justification=None
    ):
        ...
        self.loop.call_soon_threadsafe(
            self.loop.create_task, _request_suspend(pre_plan, post_plan, justification)
        )
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if (
                        coro := self._command_registry.get(
    ...
    async def _wait_for(self, msg):
        ...
        completed, pending = await asyncio.wait(
            futs, **self._loop_for_kwargs, **msg.kwargs
        )
        if pending:
    ...
    async def _prepare(self, msg):
        ...
        def done_callback(status=None):
            self.log.debug(
                "The object %r reports set is done with status %r", obj, ret.success
            )
    ...
    async def _trigger(self, msg):
        ...
        self._add_status_to_group(
            obj=obj, status_object=ret, group=group, action="trigger"
        )
        ...
        if futs:
            ...
            try:
                ...
                await self._wait_for(
                    Msg("wait_for", None, futs, timeout=msg.kwargs.get("timeout", None))
                )
    ...
    async def _checkpoint(self, msg):
        ...
        for current_run in self._run_bundlers.values():
            if current_run.bundling:
                raise IllegalMessageSequence(
                    "Cannot 'checkpoint' after 'create' and before 'save'. Aborting!"
                )
    ...
    def _add_status_to_group(
        self, obj: typing.Any, status_object: Status, group: str, action: str
    ) -> None:
        p_event = asyncio.Event(**self._loop_for_kwargs)
        pardon_failures = self._pardon_failures
        ...
        def done_callback(status=None):
            self.log.debug(
                "The object %r reports %r is done with status %r.",
                obj,
                action,
    ...
    async def _stage(self, msg):
        ...
        self._add_status_to_group(
            obj=obj, status_object=ret, group=group, action="stage"
        )
    ...
    async def _unsubscribe(self, msg):
        ...
        if (
            token := kwargs.get("token", key_absence_sentinel := object())
        ) is key_absence_sentinel:
            (token,) = arg
...
def _set_span_msg_attributes(span, msg):
    ...
    (
        span.set_attribute("msg.obj", repr(msg.obj))
        if msg.obj
        else span.set_attribute("msg.no_obj_given", True)
    )


def _default_md_validator(md):
    if "sample" in md and not (
        hasattr(md["sample"], "keys") or isinstance(md["sample"], str)
    ):
        raise ValueError(
...
def _ensure_event_loop_running(loop):
    ...
    if not loop.is_running():
        th = threading.Thread(
            target=loop.run_forever, daemon=True, name="bluesky-run-engine"
        )
        th.start()
...
def call_in_bluesky_event_loop(
    coro: typing.Awaitable[T], timeout: typing.Optional[float] = None
) -> T:
